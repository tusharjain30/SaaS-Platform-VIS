...
router.post("/", async (req, res) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.campaignLog.create({
        data: {
          campaignId,
          type: "INFO",
          message: `Bulk audience added: ${insertResult.count}`,
        },
      });
    ...
    return res.status(RESPONSE_CODES.POST).json({
      status: 1,
      message: "Bulk audience added successfully",
      statusCode: RESPONSE_CODES.POST,
      data: {
        requested: (contactIds?.length || 0) + (groupIds?.length || 0),
        added: result.added,
        skipped: contactIds.length + groupIds.length - result.added,
        totalContacts: result.totalContacts,
      },
    });
  } catch (error) {
    console.log("Bulk Add Audience Error:", error);
